...
**
*/
void __ISR(_TIMER_5_VECTOR, ipl2SOFT) Timer5ISR(void)// TA
{
...
**      The timer will generate interrupts every 3000 microseconds.
...
{
  PR5 = (int)(((float)(TMR_TIME * PB_FRQ) / 256) + 0.5);   //set period register, generates one interrupt every 3000 us
  TMR5 = 0;                           //    initialize count to 0
  T5CONbits.TCKPS = 3;                //    1:8 prescaler value
